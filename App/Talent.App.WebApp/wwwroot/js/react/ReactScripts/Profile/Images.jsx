...
export class Images extends React.Component {
    ...
    loadImages(Id) {
        ...
        const localUrlEmployer =
            "https://profilestandard.azurewebsites.net/profile/profile/getEmployerProfileImage/?id=";
        const azureUrlEmployer =
            "https://profilestandard.azurewebsites.net/profile/profile/getEmployerProfileImage/?id=";

        const localUrlImages =
            "https://profilestandard.azurewebsites.net/profile/profile/getEmployerProfileImages/?Id=";
    ...
    fileSelectedHandler(event) {
        ...
        for (let i = 0; i < event.target.files.length; i++) {
            if (
                event.target.files[i].size > this.maxFileSize ||
                this.acceptedFileType.indexOf(event.target.files[i].type) == -1
            ) {
                ...
                TalentUtil.notification.show(
                    "Exceed Maximum number of files allowable to upload",
                    "error",
                    null,
                    null
                );
                ...
                (localSelectedFile = localSelectedFile.concat(event.target.files[i])),
                    (localSelectedFileName = localSelectedFileName.concat(
                        event.target.files[i].name
                    )),
                    (localImageSrc = localImageSrc.concat(
    ...
    removeFile(event) {
        ...
        localselectedRemoveFileId = localselectedRemoveFileId.concat(
    ...
    fileUploadHandler(Id) {
        ...
        $.ajax({
            url:
              //  "https://profilestandard.azurewebsites.net/profile/profile/addEmployerProfileImages",
                "https://profilestandard.azurewebsites.net/profile/profile/addEmployerProfileImages",
            headers: {
                Authorization: "Bearer " + cookies,
            },
                ...
                showProfileImg.push(
                    <span>
                        <img
                            ...
                            className="ui small"
                            src={this.state.imageSrc[i]}
                            alt="Image Not Found"
                        />
                        <i
                            className="remove sign icon"
                            onClick={this.removeFile}
                            value={i}
                            imageid={this.state.imageId[i]}
                        ></i>
                    </span>
                );
            }
